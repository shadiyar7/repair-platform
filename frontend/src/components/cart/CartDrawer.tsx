...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose }) => {
    const { items, basePrice, vatAmount, updateQuantity, removeFromCart, isBuyback, setIsBuyback, globalDiscount, discountAmount, finalPrice } = useCart();
    const { user } = useAuth();
                            ...
                            <div className="border-t border-gray-200 py-6 px-4 sm:px-6">
                                <div className="space-y-1">
                                    <div className={`flex justify-between text-base font-medium ${discountAmount > 0 ? 'text-gray-500 line-through text-sm' : 'text-gray-900'}`}>
                                        <p>Сумма (без НДС)</p>
                                        <p>{new Intl.NumberFormat('kk-KZ', { style: 'currency', currency: 'KZT', maximumFractionDigits: 0 }).format(basePrice)}</p>
                                    </div>
                                    {discountAmount > 0 && (
                                    ...
                                    <div className="flex justify-between text-base font-medium text-gray-700 pt-1">
                                        <p>НДС (16%)</p>
                                        <p>{new Intl.NumberFormat('kk-KZ', { style: 'currency', currency: 'KZT', maximumFractionDigits: 0 }).format(vatAmount)}</p>
                                    </div>
